...
def clothing_train_and_evaluate(model, trainloader, testloader, optimizer, loss_fn, num_epochs, corr_p, rep = None, sound=10, loss_type = None, clothing = False):
    seed = 42  # You can choose any integer seed
    torch.manual_seed(seed)
    np.random.seed(seed)
    random.seed(seed)
    ...
    for epoch in range(num_epochs):
        ...
        #          print(name, param)
            ...
            #if loss_type == 'Supervised':
            #    train_targets = torch.max(targets, dim=1)[1]
            inputs, vl = inputs.to(device), vl.to(device)
